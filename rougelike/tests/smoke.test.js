...
describe("幽月魔女与月之领域", () => {
  ...
  it("月刃环：一轮 6 发细长月牙弹幕（二阶段 8 发，反弹仅二阶段 1 次）", () => {
    const { R } = loadGame();
    reachDomain(R);
    ...
    expect(R(`game.projectiles.filter(p => p.moonBlade).every(p => p.moonBounce === 0 && p.damage === 14 && Math.abs(Math.hypot(p.vx, p.vy) - 280) < 1)`)).toBe(true);
    ...
    R(`update(1/60)`);
    expect(R(`game.projectiles.filter(p => p.moonBlade).length`)).toBe(8);
    expect(R(`game.projectiles.filter(p => p.moonBlade).every(p => p.moonBounce === 1)`)).toBe(true);
  });

  it("月刃环触界反弹：法线反射且反弹次数递减（moonBounce 手动置 1 验证机制）", () => {
    const { R } = loadGame();
    reachDomain(R);
