...
describe("魔法幸存者 · 运行时帧模拟（重构回归）", () => {
  ...
  it("全敌种类 + 词缀 + Boss 在场时运行不抛错", () => {
    ...
    R(`
      ['zombie','runner','brute','wraith','pyromancer','hatchling','lavaling','bomber','warlock','moonshade'].forEach((tk,i)=>{
        const e = new Enemy(200+i*80, 300, tk, 7);
        e.affixName='迅捷'; e.affixColor='#55ddff';
        game.enemies.push(e);
      });
      ['boss','broodmother','assassin','lavabeast','turret','moonwitch'].forEach((tk,i)=>{
