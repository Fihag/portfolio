...
- **武器编队**：战前从 9 把武器中选最多 5 把入局（自动存档记忆），初始武器从编队内五选一；局内升级卡池只出现编队内武器的解锁卡
...
- **灵魂宝库**：天赋 + 圣物（11 种：`影侍守卫` 受击 45% 眩晕 0.4s（对 Boss 无效）+ 10s 双影袭，不可升级）/ 成就 8 种（含「万法归宗」：一局 5 武器全进化）/ 死神之指（手动世界坐标修复，刺客 60 半径自适应）
