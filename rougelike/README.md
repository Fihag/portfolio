...
- **难度**：简单 / 普通 / 困难 / 地狱 / **不可能**（`DIFFICULTIES` 单源 `js/config.js`）；不可能小怪 55% 词缀、10% 减伤、20% 减速免疫，Boss 减速 40%（不可能 50%，刺客免疫）；词缀表：迅捷 ×1.40 移速 / 坚韧 ×1.70 血量 / 爆裂（死亡波及 80 内 ×1.3 伤）/ 灼热（接触 2.5 伤/秒 ×2s）/ 嗜血（近战回吸 55% 伤）
...
- **幽月魔女**（特殊 Boss，整局固定两次，不计入常规轮换与出场次数，2500 血 25% 减伤）：击败第 2 / 第 5 个常规 Boss 后 10 秒降临——第 5 秒起屏幕震颤渐强，随后月之魔法阵降临（第二次降临先播镜片重组复活动画且**血量等数值 ×3（7500 血）、伤害加成单独 ×1.5**，并全阶段额外 +15% 减伤）；降临后月光锁链把玩家**拽进异空间领域**（世界外独立地图：蓝紫深空、透明地板、双黑洞环绕、符文边界环）；领域内玩家被困至 Boss 死亡，副作用：回血 -70%、移速 -10%、**血量上限锁 200、圣物与死神之指全部失效**（出场恢复）；小怪全体冻结、刷怪暂停（纯 1v1）。Boss 走位保持 300±40 距离带环绕不贴脸，**被动·穿梭**：被逼到边界且玩家逼近时镜面点对称闪现对侧（CD 30s）。领域加成一阶段 伤×1.2/速×1.3/CD-20%，**二阶段（60% 血）翻倍** 伤×1.4/速×1.6/CD-40% 并**额外 +30% 减伤（合计 55%）**且变身无敌 2.5s，之后**无敌 5s → 破绽 8s** 循环（无敌期技能照放）。六技能：月刃环（360° **6 发细长月牙**弹幕弹速 280 / 二阶段 8 发弹速 320，两阶段均**触界反弹 1 次**，伤 16）、追月弹（3 颗追踪弹 160→300 加速，**转向迟钝可被玩家急转甩掉**，伤 22）、月光洗礼（3~5 个预警圈 0.9s 后落地伤 28）、镜月分身（2~3 只一击碎镜影）、满月收缩（边界光环留 50° 缺口向圆心碾压 170 速伤 24）、升月轰炸（升空 3.5s 无法选中+免伤：天际激光扫射+弹幕雨，砸落冲击 120 半径伤 30 + 硬直 0.8s 破绽）；死亡触发**镜面碎裂**演出（白闪+镜片迸散），领域崩塌送回原地图并恢复全部状态，掉落 Boss 道具
...
- **灵魂宝库**：天赋 + 圣物（12 种：`影侍守卫` 受击 45% 眩晕 0.4s（对 Boss 无效）+ 10s 双影袭，不可升级；`抉择之冠` 升级选项恒为 6 个）/ 成就 8 种（含「万法归宗」：一局 6 武器全进化）/ 死神之指（手动世界坐标修复，刺客 60 半径自适应）/ 底部**兑换码**按钮：弹窗输入 `Fihag+数字`（如 `Fihag100`）即获得对应数量灵魂碎片，计入累计
...
素材加载按环境自动选择通道：`http(s)` 下用 `fetch` + `decodeAudioData`，享受完整分层（循环、低通、二阶段强度抬升）；`file://` 直开（双击 index.html）时浏览器会拦截 `fetch`，此时退回 XHR，再退回原生 `<audio>` 元素播放——**双击打开同样有音乐**，只是音量随分层档位变化、不做低通。

素材全部缺失或解码失败（如旧版 Safari 不解 ogg）时自动回退到 `02-utils.js` 内的合成音效，不会报错或静音；每个音效名都有合成兜底。替换素材只需覆盖 `audio/moon/` 下同名文件，代码零改动。
